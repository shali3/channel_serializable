...
class _FieldSet implements Comparable<_FieldSet> {
  ...
  factory _FieldSet(FieldElement classField, FieldElement superField) {
    ...
    // Prefer the field that's annotated with `ChannelKey`, if any.
    // If not, use the class field.
    final fieldHasChannelKey =
        fields.firstWhere(hasChannelKeyAnnotation, orElse: () => fields.first);

    return _FieldSet._(fieldHasChannelKey, sortField);
  }
