...
bool hasChannelKeyAnnotation(FieldElement element) =>
    channelKeyAnnotation(element) != null;
...
void throwUnsupported(FieldElement element, String message) =>
    throw InvalidGenerationSourceError(
        'Error with `@ChannelKey` on `${element.name}`. $message',
        element: element);
...
/// Return an instance of [ChannelSerializable] corresponding to a the provided
/// [reader].
ChannelSerializable _valueForAnnotation(ConstantReader reader) =>
    ChannelSerializable(
...
/// Returns a [ChannelSerializable] with values from the [ChannelSerializable]
/// instance represented by [reader].
///
/// For fields that are not defined in [ChannelSerializable] or `null` in [reader],
/// use the values in [config].
ChannelSerializable mergeConfig(
    ChannelSerializable config, ConstantReader reader) {
  final annotation = _valueForAnnotation(reader);

  return ChannelSerializable(
...
/// If the enum value is annotated with [ChannelKey], then the `name` property is
