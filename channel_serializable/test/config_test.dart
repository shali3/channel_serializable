...
import 'package:channel_serializable/builder.dart';
import 'package:channel_serializable/src/channel_serializable_generator.dart';
...
void main() {
  test('fields in ChannelSerializable are sorted', () {
    expect(generatorConfigDefaultJson.keys,
        orderedEquals(generatorConfigDefaultJson.keys.toList()..sort()));
  });

  test('empty', () async {
    final builder = channelSerializable(BuilderOptions.empty);
    expect(builder, isNotNull);
  });

  test('valid default config', () async {
    final builder =
        channelSerializable(BuilderOptions(generatorConfigDefaultJson));
    expect(builder, isNotNull);
  });
  ...
  test('valid, non-default config', () {
    ...
    final builder =
        channelSerializable(BuilderOptions(generatorConfigNonDefaultJson));
    expect(builder, isNotNull);
  });

  test('config is null-protected when passed to ChannelSerializableGenerator',
      () {
    final nullValueMap = Map.fromEntries(
        generatorConfigDefaultJson.entries.map((e) => MapEntry(e.key, null)));
    final config = ChannelSerializable.fromJson(nullValueMap);
  ...
  test('readme config', () async {
    ...
    expect(
      ChannelSerializable.fromJson(configMap).toJson(),
      generatorConfigDefaultJson,
    );

    final builder = channelSerializable(BuilderOptions(configMap));
    expect(builder, isNotNull);
  });
  ...
  test('unsupported configuration', () async {
    final matcher = isA<StateError>().having(
      (v) => v.message,
      'message',
      'Could not parse the options provided for `channel_serializable`.\n'
    ...
    expect(
        () => channelSerializable(
            const BuilderOptions({'unsupported': 'config'})),
        throwsA(matcher));
  });
...
$lastLine''',
        ...
        expect(() => channelSerializable(BuilderOptions(config)),
            throwsA(matcher));
      });
    }
  });
