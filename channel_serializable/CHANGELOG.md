...
- Require `package:channel_annotation` `^3.0.0`.
- Added support for `ChannelSerializable.ignoreUnannotated`.
- Added support for `ChannelKey.unknownEnumValue`.
...
- Removed support for `ChannelSerializable.useWrappers`.
- Removed support for `ChannelSerializable.generateToJsonFunction`.
- Removed support for `encodeEmptyCollection`.
- If a field has a conversion function defined – either 
  `ChannelKey.toJson` or a custom `JsonConverter` annotation – don't intercept
...
- If a field has a conversion function defined – either `ChannelKey.toJson` or a
...
* Support `package:channel_annotation` `>=2.1.0 <2.3.0`.
...
* Added support for `encodeEmptyCollection` on `ChannelKey` and `ChannelSerializable`.
...
* Log warnings when `ChannelKey.defaultValue` is set with other fields.
...
* Support all `build.yaml` configuration options on classes by adding a number
  of fields to `ChannelSerializable`: `anyMap`, `checked`, `explicitToJson`,
...
* Build configuration
  ...
  * You can now configure all settings exposed by the `ChannelSerializable`
...
* `channel_serializable.dart`

  * **BREAKING** `ChannelSerializableGenerator` now exposes a `config` property
    of type `ChannelSerializable` instead of individual properties for `checked`,
...
* Add support for types annotated with classes that extend `JsonConverter` from
  `package:channel_annotation`.

* Export the following `TypeHelper` implementations in
  `package:channel_serializable/type_helper.dart`:
...
* Supports class-static functions for `toJson` and `fromJson` on `ChannelKey`.
...
* Added support for automatically converting field names to JSON map keys as
  `kebab-case` or `snake_case` with a new option on the `ChannelSerializable`
  annotation.
...
* `ChannelSerializableGenerator.generateForAnnotatedElement` now returns
...
* If a manual build script is used the `channel_serializable` builder must be
...
* Added support for `ChannelKey.required`.
  ...
  * Will be captured and wrapped in a `CheckedFromJsonException` if
    `checked` is enabled in `channel_serializable`.

* Added `ChannelKey.disallowNullValue`.
...
* Added missing `checked` parameter to the
  `ChannelSerializableGenerator.withDefaultHelpers` constructor.

* Added `explicit_to_json` configuration option.
  * See `ChannelSerializableGenerator.explicitToJson` for details.

* Added `generate_to_json_function` configuration option.
  * See `ChannelSerializableGenerator.generateToJsonFunction` for details.

## 0.5.6

* Added support for `ChannelSerializable.disallowUnrecognizedKeys`.
...
* Added support for `ChannelKey.defaultValue`.

* `enum` deserialization now uses helpers provided by `channel_annotation`.
...
* If `ChannelKey.fromJson` function parameter is `Iterable` or `Map` with type
...
* `ChannelKey.fromJson`/`.toJson` now support functions with optional arguments.

## 0.5.2

* If `ChannelKey.fromJson`/`toJson` are set, apply them before any custom
  or default `TypeHelper` instances. This allows custom `DateTime` parsing,
  by preempting the existing `DateTime` `TypeHelper`.

## 0.5.1

* Support new `fromJson` and `toJson` fields on `ChannelKey`.
...
* Private and ignored fields are now excluded when generating serialization and
  deserialization code by using `@ChannelKey(ignore: true)`.
...
* **NEW** top-level library `channel_serializable.dart`.

  * Replaces now deprecated `generators.dart` to access
  `ChannelSerializableGenerator` and `JsonLiteralGenerator`.
...
* **BREAKING** The deprecated `annotations.dart` library has been removed.
  Use `package:channel_annotation` instead.
...
* `ChannelSerializableGenerator` now supports an optional `useWrappers` argument
...
* Support the `nullable` field on the `ChannelSerializable` class annotation.
...
* Moved the annotations in `annotations.dart` to `package:channel_annotations`.
...
* **BREAKING** Types are now segmented into their own libraries.
  ...
  * `package:channel_serializable/generators.dart` contains `Generator`
    implementations.

  * `package:channel_serializable/annotations.dart` contains annotations.
    This library should be imported with your target classes.

  * `package:channel_serializable/type_helpers.dart` contains `TypeHelper` classes
...
* **BREAKING** `ChannelKey.jsonName` was renamed to `name` and is now a named
  parameter.
