...
- Deprecate members that will be removed in `channel_annotation` `3.0.0` and that
  are not supported in `channel_serializable` `3.0.0`.
...
* **Potentially Breaking** `JsonSerializable` no longer sets default values for
  ...
  version of `package:channel_serializable`, but it may cause issues if class is
...
* Removed `$checkAllowedKeys`, `$enumDecode` and `$enumDecodeNullable` which are
  no longer needed by the latest release of `package:channel_serializable`.
...
* Added `$checkKeys` helper function and deprecated `$checkAllowedKeys`.
  Upgrading to the latest `channel_serializable` and re-running your build will
